...
{
    const int range = 1;
    const float y_interval = 0.00525;
    const float cbcr_interval = 0.025;
    ...
    vec4 cbcrTex = texture(sampler2D(Texture2D, Sampler), fsin_UV + (pixelated_coords - fsin_UV) * 0.125, 1.0);
